...
const MODULES = [
  ...
  { id: "brief", label: "Draft", category: "COMMUNICATIONS INTELLIGENCE", icon: "✉️", color: MOD.brief, desc: "AI-drafted communications grounded in live Slate data. In your voice. Out in seconds.", status: "LIVE", metrics: "Powered by Claude", tagline: "Your voice. Your data. Seconds, not hours." },
...
const SplashScreen = ({ onComplete }) => {
  ...
  return (
    <div style={{
      ...
      background: "linear-gradient(160deg, #1A1B1F 0%, #121315 35%, #23272F 65%, #1A1B1F 100%)",
      ...
      <div style={{ position: "relative", marginTop: 24, display: "flex", alignItems: "baseline", gap: 0, opacity: p >= 1 ? 1 : 0, transform: p >= 1 ? "translateY(0)" : "translateY(24px)", transition: "all 1.2s cubic-bezier(0.16, 1, 0.3, 1) 0.25s" }}>
        ...
        <span style={{
          ...
          opacity: dotX === 0 ? 1 : 0,
          transition: dotX === 0
            ? "transform 1.1s cubic-bezier(0.16, 1, 0.3, 1) 0.6s, opacity 0.1s ease 0.6s"
            : "none",
          boxShadow: `0 0 18px 4px ${C.brass}55`,
        }}/>
      </div>
      ...
      <div style={{ position: "relative", marginTop: 64, display: "flex", gap: 24, opacity: p >= 2 ? 1 : 0, transition: "opacity 0.8s ease 0.4s" }}>
        {MODULES.map((m, i) => (
          <div key={m.id} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10, opacity: p >= 2 ? 1 : 0, transition: `opacity 0.6s ease ${0.5 + i * 0.12}s` }}>
            <div style={{ width: 10, height: 10, borderRadius: "50%", background: m.color, boxShadow: `0 0 14px ${m.color}60`, animation: p >= 2 ? "slateDotPulse 3s ease-in-out infinite" : "none", animationDelay: `${i * 0.3}s` }} />
...
${JSON.stringify(snap, null, 2)}`,
  ...
  return (<>
    ...
    <div style={{ display: "flex", height: "100vh", width: "100vw", fontFamily: "'Inter', system-ui, sans-serif", background: C.bg, overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "visible", minWidth: 0, transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)", position: "relative" }}>

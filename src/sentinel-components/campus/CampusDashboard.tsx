...
Share
Max
Let me read through this file to understand the full campus page structure.
I've reviewed the full campus page structure, noting its sub-components and layout. My next step is to rewrite the campus section to match the D2 mockup, ensuring component integration aligns with the design.

Max
Perfect — I've read the full SentinelApp.tsx (748 lines). I can see exactly how the campus page works:
Current structure (lines 545-620): A single-column 800px max-width layout that stacks 6 "chapters" vertically:
SituationCard (headline + risk score)
MorningBriefing (AI assessment)
RightNowBar + IceIntelligence
LastNight + IncidentList
CampusMap
CampusToolsDrawer (collapsed: contagion, forecast, corridors, emergency, IntelQuery)
D2 mockup target: A structured layout with:
Top: Campus header with name, status badge, KPI strip (incidents 7D, violent 24H, contagion zones), sparkline, weather
Middle: Full-width AI Intelligence Briefing with action buttons
Bottom: Three-column grid — Incident Timeline (40%) | Campus Map (30%) | Risk Profile (30%)
Footer: Ask Slate input
Here's my approach: I'll create a new CampusDashboard.tsx component that wraps your existing sub-components in the D2 layout. It'll import and reuse SituationCard, MorningBriefing, IncidentList, CampusMap, ContagionPanel, etc. — but arrange them in the three-column grid. This way you don't lose any existing logic.
Let me write it now.
I analyzed the current campus page structure and D2 mockup layout. Next, I'll create the CampusDashboard component to arrange existing modules into the new three-column D2 design.

Max
Here's the complete CampusDashboard.tsx — a drop-in component that matches the D2 mockup. Here's how it maps to the design:
Layout matching the D2 mockup:
D2 Mockup Section
Component
Description
Top bar — "ENGLEWOOD" + ELEVATED badge + KPIs + sparkline + weather
CampusHeader
Campus name in 32px Playfair Display, status badge, 3 KPI cards (Incidents 7D, Violent 24H, Contagion Zones), 7-day sparkline, weather. Color-coded top border (green/amber/red by risk).
AI Intelligence Briefing — afternoon assessment + action buttons
AIBriefing
Full-width card with "✦ AI Intelligence Briefing" header, time-of-day assessment label (AFTERNOON ASSESSMENT), Claude-generated narrative prose, and three action buttons: Notify Deans, Contact Legal, View Full Briefing.
ICE Alert (conditional)
Inline purple banner
Only shows when ICE alerts are active. Purple background with lock icon and protocol instructions.
Three-column grid — Timeline | Map | Risk Profile
Grid layout
Equal thirds. Left: IncidentTimeline (24h incidents with severity dots, timestamps, distance, source). Center: CampusMapSection (wraps your existing CampusMap component with legend). Right: RiskProfile (5 metric bars — Violence Index, Contagion Risk, Trend Direction, Source Coverage, Weather Impact — plus network comparison dot).
Contagion Zones (conditional)
Inline section
Expandable zone cards with phase labels, distance, elapsed time, countdown bars.
Ask Slate
AskSlate
Input bar with 3 suggested questions as pill buttons.
How to integrate:
This is a standalone component — import it and pass the same props your current campus page uses. In your SentinelApp.tsx, replace the current chapter-stack rendering (the <SituationCard>, <MorningBriefing>, <LastNight>, <IncidentList>, <CampusMap> vertical stack) with:
TSX
...
<CampusDashboard
  ...
  onBeginProtocol={handleProtocol}
/>
Key design choices:
Same warm Slate palette — cream background (#FAF8F5), white cards, chalk borders, Playfair Display serif for headlines, Inter for body
Reuses your existing CampusMap component — just wraps it in the new card layout
AI briefing calls your existing /api/anthropic-proxy — same Claude integration, just with a refined campus-specific prompt
Risk Profile bars are computed from the same CampusRisk data — no new data sources needed
Responsive three-column grid at the bottom matches D2's 40/30/30 split (I used equal thirds for simplicity — easy to adjust)
Both the Network page (Concept E) and this Campus page (D2) now share the same design DNA — Playfair Display headlines, warm cream backgrounds, red section accents, card-based layouts with chalk borders. They flow into each other naturally.
CampusDashboard.tsx
Code · 35.12 KB
View all files in this task
How was this result?
how do i deploy this in here?
Max
I can see your full repo structure. Here's the deployment plan — it's two steps:
Step 1: Add the new file
Go to: https://github.com/mikemadden333/slate/tree/main/src/sentinel-components/campus
Click "Add file" → "Create new file"
Name it: CampusDashboard.tsx
Paste the entire contents of the file I gave you
Click "Commit changes"
Step 2: Update SentinelApp.tsx to use it
This is where it gets tricky — you need to replace the campus rendering section in SentinelApp.tsx. The current code (around lines 545-620 ) stacks the chapter components vertically. You'd replace that with the CampusDashboard import and render.
But honestly — this is a big edit to do in GitHub's web editor. The SentinelApp.tsx is 748 lines and the campus rendering section is deeply nested.
Better approach: Would you like me to rewrite the full SentinelApp.tsx with the CampusDashboard integrated? Then you can do the same paste-and-replace approach — select all, delete, paste the new version. That way there's zero risk of a partial edit breaking things.
Want me to do that? I'll need you to paste the current SentinelApp.tsx contents again (the context was compressed and I lost it), or I can work from what I remember of the structure.
SentinelApp.tsx yes
Max
Can you paste the full contents of SentinelApp.tsx here? I need to see the current code to rewrite it with CampusDashboard integrated. Just open the file on GitHub, click "Raw", select all, and paste it here.
...
Pitch Deck Creation for Madden Education Advisory and Slate - Manus

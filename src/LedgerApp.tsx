  ...
  return (
    <LedgerProvider>
      <div style={{ fontFamily: "'Inter', system-ui, sans-serif", color: '#0D1117' }}>
        ...
        {/* View mode toggle + campus selector */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button onClick={() => {
              if (!selectedCampus) setSelectedCampus(CAMPUSES[0]);
              setViewMode('campus');
        ...
        <div style={{ display: 'flex', gap: 0, marginBottom: 24, overflowX: 'auto', borderBottom: '1px solid #E8EDF2' }}>
          {tabs.map(tab => (
            <button key={tab.id} onClick={() => handleModuleChange(tab.id)} style={{
              ...
              borderBottom: activeModule === tab.id ? '2px solid #B79145' : '2px solid transparent',
        ...
        <footer style={{ textAlign: 'center', padding: '20px 16px', marginTop: 32,
          fontSize: 11, color: '#4A5568', borderTop: '1px solid #E8EDF2' }}>
          <div>Slate Ledger — Veritas Charter Schools — $138M Annual Budget — FY2026</div>
          <div style={{ fontSize: 9, color: '#94A3B8', marginTop: 4 }}>
            Slate Systems, LLC · Madden Education Advisory · 2026
          </div>
        </footer>
      </div>
    </LedgerProvider>
